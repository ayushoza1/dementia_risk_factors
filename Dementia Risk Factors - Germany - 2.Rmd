...
sev_imp_cutoff <- mean(reduced_dataset$cogscore) - sd(reduced_dataset$cogscore) ## Calculate the cutoffs for severe impairment
...
final_dataset <- reduced_dataset[ , c("isced1997_r", "country", "age", "bmi2", "smoking", "ever_smoked","eurod", "female", "cog_cat", "euro1", "euro2", "euro3", "euro4", "euro5", "euro6", "euro7", "euro8", "euro9", "euro10", "euro11", "euro12", "casp", "br010_mod", "wave", "country_name", "mother_alive", "father_alive", "siblings_alive", "ch001_", "ch021_mod", "iv009_mod", "hhsize", "ch007_km", "chronic_mod", "mobilityind", "lgmuscle", "bmi", "cogscore", "ever_smoked", "eduyears_mod")]
...
We create an additional data frame with ordinal and continuous variables to plot a correlation matrix which will be used and analysed when deciding variables for stochastic regression imputation.

```{r}

corr_dataset <- final_dataset[,c("eduyears_mod", "age","eurod", "cogscore","casp", "br010_mod", "wave", "siblings_alive", "ch001_", "ch021_mod", "hhsize", "chronic_mod", "mobilityind", "lgmuscle", "bmi")] ## new dataset created for correlation matrix

corr_dataset <- data.table(corr_dataset) ## convert to datatble

numcols <- sapply(corr_dataset, is.numeric) 
cor.dementia <- corr_dataset[, ..numcols] %>% #subset of numeric columns
            cor(use="pairwise.complete")


corrplot(cor.dementia, # order the variables by correlation clusters to 
         ## block structure is highlighted
...
Plots for the stochastic regression imputation of BMI comparing predicted, observed and residual values ensuring all assumptions are met.

```{r}

plot_data_bmi <- data.frame(Predicted_value = predsri2,  
                       Observed_value = bmi_data$bmi) ## Create data frame of predicted and observed values

plot_data_bmi <- plot_data_bmi[complete.cases(plot_data_bmi),] ## generate all complete cases

BMI_plot <- data.frame(resid = resid(model2), pred = plot_data_bmi$Predicted_value) ## Create data frame of predicted and residual values


# plot predicted values and actual values
ggplot(BMI_plot, aes(x = pred, y = resid)) +
                  geom_point() +
                 geom_abline(intercept = 0, slope = 0, color = "green") + xlab("Fitted Values") + ylab("Residuals") + ggtitle("BMI Stochastic Regression Imputation Results") + xlim(0, 48)
...
Plots for the stochastic regression imputation of CASP comparing predicted, observed and residual values ensuring all assumptions are met.

```{r}
plot_data <- data.frame(Predicted_value = predsri,  
                       Observed_value = casp_data$casp) ## Create a data frame storing the predicted and observed CASP values

plot_data <- plot_data[complete.cases(plot_data),] ## Only keep the complete CASP and predicted CASP cases 
  
# plot predicted values and actual values
ggplot(plot_data, aes(x = Predicted_value, y = Observed_value)) +
                  geom_point() +
                 geom_abline(intercept = 0, slope = 1, color = "green") + xlab("Predicted Values") + ylab("Observed Values") + ggtitle("CASP Score Stochastic Regression Imputation Results") + xlim(0, 48)
```
...
Plot the distribution of new variables created by country to get a better feel for the data

```{r}

social_family_plot <- final_dataset_relevent_vars[final_dataset_relevent_vars$country_name %in% c("Germany", "Estonia", "Spain", "Czechia", "Israel"), c("social_status", "family_status","country_name")] ## Create a new data frame for family and social status, keeping the countries we examine later on.

ggplot(social_family_plot, aes(x = social_status, fill = country_name)) +
  geom_bar() + ylab("Count") + xlab("Index") + ggtitle("Distribution of Social Status") + guides(fill=guide_legend(title="Country")) ## Plot distribution of social status

ggplot(social_family_plot, aes(x = family_status, fill = country_name)) +
  geom_bar() + ylab("Count") + xlab("Generations Alive") + ggtitle("Distribution of Family Status") + guides(fill=guide_legend(title="Country"))  ## Plot distribution of family status
...
For a given country - in this case Germany store all complete cases from each wave to help us identify a specific wave to use for the analysis. 
...
country_dataset <- final_dataset_relevent_vars[final_dataset_relevent_vars$country_name == "Germany", ] ## Store all waves of the country due to be analysed
...
wave_analysis <- data.frame(wave = sort(unique(country_dataset$wave)), Complete_cases = c(wave1, wave2, wave4, wave5, wave6, wave8), surveys = c(wave1.num, wave2.num, wave4.num, wave5.num, wave6.num, wave8.num) ) ## Store in a dataframe so we can plot with ggplot

ggplot(wave_analysis, aes(x=wave, y=Complete_cases)) + 
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 45))  ## PLot graph of complete cases 

ggplot(wave_analysis, aes(x=wave, y=surveys)) + 
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 45))  ## Plot graph of total respondants 
...
country_wave_dataset <- country_dataset[country_dataset$wave == 5,] ## Reduce dataset further to keep records with just the wave of interest 
...
social <- final_dataset[final_dataset$wave == 5 & final_dataset$country_name == "Germany", c("iv009_mod", "hhsize", "ch007_km")] ## Create a new social dataset with variables of interest 
...
country_wave_dataset$depression_band <- rep(NA, nrow(country_wave_dataset))
country_wave_dataset$depression_band[country_wave_dataset$eurod == 0 | country_wave_dataset$eurod == 1 ] <- 1
country_wave_dataset$depression_band[country_wave_dataset$eurod == 2 | country_wave_dataset$eurod == 3 |country_wave_dataset$eurod == 4] <- 2
country_wave_dataset$depression_band[country_wave_dataset$eurod == 5 | country_wave_dataset$eurod == 6 |country_wave_dataset$eurod == 7 | country_wave_dataset$eurod == 8] <- 3
country_wave_dataset$depression_band[country_wave_dataset$eurod == 9 | country_wave_dataset$eurod == 10 |country_wave_dataset$eurod == 11 | country_wave_dataset$eurod == 12 ] <- 4

## Ensure eurod is a factor variables 
country_wave_dataset$eurod <- factor(country_wave_dataset$eurod, ordered = TRUE)
country_wave_dataset$depression_band <- factor(country_wave_dataset$depression_band, ordered = TRUE)
...
Changing the interval conditions for some bands may have a large impact in the average cognitive score. We aim to get non-disjoint plots.
...
depression_v_cogscore <- aggregate(country_wave_dataset$cogscore, list(country_wave_dataset$depression_band), FUN=mean) ## Look at average cog score across depression bands
...
ggplot() + geom_bar(aes(x = country_wave_dataset$education_band),color="darkblue", fill="lightblue")+
  labs(x ="Education Band", y = "Count") + ggtitle("Education Band Distribution for Germany") ## Count of values in Education bands
...
ggplot() + geom_bar(aes(x = country_wave_dataset$depression_band),color="darkblue", fill="lightblue")+
...
labs(x ="Qality of life index band", y = "Count") + ggtitle("Quality of Life Distribution for Germany")## Count of values in casp bands
...
data4 <- country_wave_dataset[, c("age", "female","cog_cat","alcohol", "education_band", "depression_band", "casp_band", "social_status", "family_status", "bmi2", "chronic_mod", "ever_smoked")] ## Prepare larger dataset for DAG construction
data5 <- country_wave_dataset[, c("age", "female","cog_cat","alcohol", "education_band", "depression_band", "casp_band", "family_status", "bmi2", "ever_smoked")] ## Prepare larger dataset for DAG construction
```
...
myblacklist4 = matrix(c("cog_cat","age",
                    ...
                    "ever_smoked", "family_status",
                    
                    "depression_band", "education_band",
                    "depression_band", "family_status", 
                    
                    "casp_band", "family_status",
                    "casp_band", "social_status", 
                    "casp_band", "bmi2", 
                    "casp_band", "education_band", 
                    "casp_band", "ever_smoked",
                    "casp_band", "chronic_mod",
...
## Build DAG
dag7.iamb = gs(data4, blacklist = myblacklist4, whitelist = mywhitelist,test="mi", alpha = 0.05)
...
boot <- boot.strength(data4_narm, R = 500, algorithm = "iamb", algorithm.args = list(blacklist = myblacklist4, whitelist = mywhitelist)) ## Run the bootstrapping function with 500 samples from the data to create 500 DAGs

avg.boot <- averaged.network(boot, threshold = 0.85) ## Create an average DAG

avg.boot2 <- skeleton(avg.boot) ## Generate the average skeleton structure

## Plot both DAG and the skeleton structure
graphviz.plot(avg.boot)
graphviz.plot(avg.boot2)

options(warn = defaultW) ## Enable warnings again
```
...
ci.test("cog_cat", "casp_band", c("depression_band"), test = "mi", data = data5) ## Ci test between cognitive category and CASP given depression band

```

To finalise DAGs we run CI tests and add any arcs that we belive should be in the DAG but have been missed out. We use mi tests and mi-sp tests for smaller datasets. 

```{r}

ci.test("female", "cog_cat", c("depression_band"), test = "mi", data = data5) ## CI test between gender and cognitive category given depression band
...
ci.test("cog_cat", "depression_band", c("female"), test = "mi", data = data5) ## CI test between cognitive category and depression band given gender
...
dsep(dag7.gs, 'depression_band', 'cog_cat', "female") ## Check if depression band, cognitive category and gender are d seperted 
...
dsep(dag7.gs, 'depression_band', 'cog_cat', "casp_band") ## Check if depression band, cognitive category and quality of life index are d seperted 
...
bn.bayes = bn.fit(dag6.iamb,data=data4, method = "bayes", iss = 10) ## Generate marginal distributions
...
bn.fit.barchart(bn.bayes$cog_cat, main = "Conditional Probabilities for Risk of Dementia - Germany", ylab = "Dementia") ## Generate full conditional probability bar chart

```
Generate conditional probability graphs with probabilty of dementia varying across age and education levels 

```{r}
p1 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
                     y=matrix(t(bn.bayes$cog_cat$prob[,,1]), ncol =1), color = rep(levels(country_wave_dataset$cog_cat), each = length(levels(country_wave_dataset$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Germany - Primary Education")
p2 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
                     y=matrix(t(bn.bayes$cog_cat$prob[,,2]), ncol =1), color = rep(levels(country_wave_dataset$cog_cat), each = length(levels(country_wave_dataset$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Germany - Secondary Education")
p3 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
                     y=matrix(t(bn.bayes$cog_cat$prob[,,3]), ncol =1), color = rep(levels(country_wave_dataset$cog_cat), each = length(levels(country_wave_dataset$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Germany - Tertiary Education")
```
...
Cross validation datasets across countries - checking what country performed the best 

```{r}

CV_df <- data.frame(country= c("Germany", "Spain", "Czech", "Estonia", "Israel"), IAMB = c(12.77576, 13.12822, 13.13899, 12.72618, 13.5069), GS = c(12.81397, 13.17125, 13.16208, 12.76424, 13.56127)) ## Generate CV dataset to plot a bar chart


CV_df <- melt(CV_df) ## reshape the dataset 
names(CV_df)[3] <- "percent"
ggplot(CV_df, aes(x = country, y= percent-12.5, fill = variable), xlab="Age Group") +
   geom_bar(stat="identity", width=.5, position = "dodge") + ylab("Standardized Expected Loss (EL - 12.5)") + guides(fill=guide_legend(title="Algorithm Type")) + xlab("Country") + ggtitle("Cross Validation Results") ## Plot the CV data accross countries
```

Plot the BIC and BDE scores across the datasets to compare across countries

```{r}


df <- data.frame(x=c(-24349.74, -20160.9, -23728.38, -15929.99, -10738.86),
                 y=c(-24347.11, -20199.68, -23598.4, -15789.08, -10678.19),
                 z=c("Germany", "Spain", "Czech", "Estonia", "Israel")) ## Create dataframe of countries and BIC/BDE scores


## Plot scatter plot
ggplot(df, aes(x,y)) +
  geom_point() +
  geom_text_repel(aes(label = z)) + ylab("BIC Score") + xlab("BDE Score") + ggtitle("BIC Vs BDE Scores Accross Countries") 

```

Generate marginal distributions of education bands

```{r}

bn.bayes$education_band ## show conditional probability tables for education

```

Generate marginal distributions of education band in a bar chart form

```{r}
bn.fit.barchart(bn.bayes$education_band, main = "Conditional Probabilities for Education Band - Germany", ylab = "Education Level") ## bar chart for conditional probabilities for education bands
```
