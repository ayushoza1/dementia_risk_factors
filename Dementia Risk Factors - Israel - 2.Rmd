...
family_aggr_1 = aggr(family_na, col=mdc(1:2), numbers=TRUE, sortVars=TRUE, labels=names(family_na), cex.axis=.7, gap=3, ylab=c("Proportion of missingness","Missingness Pattern"))
...
country_dataset <- final_dataset_relevent_vars[final_dataset_relevent_vars$country_name == "Israel", ] ## Store all waves of the country due to be analysed
...
wave_analysis <- data.frame(wave = sort(unique(country_dataset$wave)), Complete_cases = c(wave1, wave2, wave5, wave6, wave8), surveys = c(wave1.num, wave2.num, wave5.num, wave6.num, wave8.num) ) ## Store in a dataframe so we can plot with ggplot
...
country_wave_dataset <- country_dataset[country_dataset$wave == 2,] ## Reduce dataset further to keep records with just the wave of interest 
...
social <- final_dataset[final_dataset$wave == 2 & final_dataset$country_name == "Israel", c("iv009_mod", "hhsize", "ch007_km")] ## Create a new social dataset with variables of interest 
...
ci.test("cog_cat", "social_status", test = "mc-mi", data = data4) ## CI test between social status and cognitive category 
...
ci.test("cog_cat", "casp_band", c("depressionband"),test = "mc-mi", data = data4) ## CI test between quality of life index and cognitive category given depression band
...
p1 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
  ...
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Israel\nSecondary Education - Depression Band 1")
...
Generate conditional probability graphs with probabilty of dementia varying across age and education levels bands for a set depression band
