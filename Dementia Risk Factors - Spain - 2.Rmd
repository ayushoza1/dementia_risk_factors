...
reduced_dataset$cog_cat <- "Normal"
...
country_dataset <- final_dataset_relevent_vars[final_dataset_relevent_vars$country_name == "Spain", ] ## Store all waves of the country due to be analysed
...
country_wave_dataset <- country_dataset[country_dataset$wave == 5,]

md_pattern(country_wave_dataset,pattern=FALSE,color=c('#34111b','#e30f41'))

family_aggr_1 = aggr(country_wave_dataset, col=mdc(1:2), numbers=TRUE, sortVars=TRUE, labels=names(country_wave_dataset), cex.axis=.7, gap=3, ylab=c("Proportion of missingness","Missingness Pattern"))
...
Look into the missingess of our final complete dataset used in DAGs. We rename varaibles to make it easier to read and hence we create a new dataframe. 
...
We now look at converting all variables from continuous variables to discrete variables. This is done via a vartiety of techniques, from interval generation to changnig the type of variable from an ordinal variable to a factor variable.
...
Changing the interval conditions for some bands may have a larrge impact in the average congnitive score. We aim to get non-disjoin plots.
...
labs(x ="Alcohol Category", y = "Count") + ggtitle("Alcohol Category Distribution for Spain") 
...
```
We are considering two types of constraint based algorthm so we evalulate the data with a gs algorithm and compare it to the iamb algorithm.
...
## Build DAG
dag6.gs = gs(data4, blacklist = myblacklist4, whitelist = mywhitelist,test="mi", alpha = 0.05)
graphviz.plot(dag7.gs)

## Test BIC and BDE scores
score(dag6.gs,data=data4_narm,type="bic")
score(dag6.gs,data=data4_narm,type="bde")
...
```{r}
bn.cv(data4_narm, bn = "iamb", algorithm.args = list(blacklist = myblacklist4, whitelist = mywhitelist, test="mi"), k=3)
...
ci.test("cog_cat", "casp_band", c("depression_band"), test = "mi", data = data5) ## CI test between cognitive category and quality of life given depression
...
ci.test("female", "cog_cat", c("depression_band"), test = "mi", data = data5) ## CI test between cognitive category and gender given gender
...
ci.test("cog_cat", "bmi2", test = "mi", data = data5) ## CI test between cognitive category and bmi band 
...
ci.test("cog_cat", "ever_smoked", test = "mi", data = data5) ## CI test between cognitive category and smoking 
...
bn.bayes = bn.fit(dag6.gs,data=data4, method = "bayes", iss = 10) ## Generate marginal distributions
...
bn.fit.barchart(bn.bayes$cog_cat, main = "Conditional Probabilities for Risk of Dementia - Spain", ylab = "Dementia") ## Generate full conditional probability bar chart
...
p1 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
  ...
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Spain - Primary Education")
p2 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
  ...
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Spain - Secondary Education")
p3 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
  ...
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Spain - Tertiary Education")
```
