...
Check Romania as a country for records

```{r}

easySHARE_rel8_0_0["country_name" == "Romania", ] ## Check Romania as a country for records
...
Based on the variables in the easyShare data set the storage vectors are filled with a yes (1) or no (0) binary variable. The family_status variable is created and generated by summing the storage vectors. 
...
family <- final_dataset[, c("mother_alive", "father_alive", "siblings_alive", "ch001_", "ch021_mod")]
family_na <- final_dataset[, c("mother_alive", "father_alive", "siblings_alive", "ch001_", "ch021_mod")]

family_aggr_1 = aggr(family, col=mdc(1:2), numbers=TRUE, sortVars=TRUE, labels=names(family_na), cex.axis=.7, gap=3, ylab=c("Proportion of missingness","Missingness Pattern"))
...
parents_alive[family$father_alive == 5 & family$mother_alive == 5] <- 0
parents_alive[family$father_alive == 1 | family$mother_alive == 1] <- 1
siblings_alive[family$siblings_alive > 0] <- 1
siblings_alive[family$siblings_alive == 0] <- 0

children_alive[family$siblings_alive > 0] <- 1
children_alive[family$siblings_alive == 0] <- 0

grandchildren_alive[family$ch021_mod > 0] <- 1
grandchildren_alive[family$ch021_mod == 0] <- 0


family_status <- parents_alive +  siblings_alive + children_alive + grandchildren_alive


family_status_df <- data.frame(family_status)

ggplot(family_status_df, aes(x = family_status)) +
  geom_bar()

final_dataset$family_status <- family_status
...
social <- final_dataset[, c("iv009_mod", "hhsize", "ch007_km")]
social <- replace(social, social < 0, NA)

social_aggr_1 = aggr(social, col=mdc(1:2), numbers=TRUE, sortVars=TRUE, labels=names(social), cex.axis=.7, gap=3, ylab=c("Proportion of missingness","Missingness Pattern"))

social$ch007_km[social$ch007_km == 5] <- 0

social_status <- (5 - social$iv009_mod) + social$hhsize + social$ch007_km

social_status_df <- data.frame(social_status)

ggplot(social_status_df, aes(x = social_status)) +
  geom_bar()

final_dataset$social_status <- social_status

social_aggr_2 = aggr(social_status, col=mdc(1:2), numbers=TRUE, sortVars=TRUE, labels=names(social_status), cex.axis=.7, gap=3, ylab=c("Proportion of missingness","Missingness Pattern"))
...
country_dataset <- final_dataset_relevent_vars[final_dataset_relevent_vars$country_name == "Estonia", ] ## Store all waves of the country due to be analysed
...
wave_analysis <- data.frame(wave = sort(unique(country_dataset$wave)), Complete_cases = c(wave4, wave5 ,wave6, wave8), surveys = c(wave4.num, wave5.num, wave6.num, wave8.num) ) ## Store in a dataframe so we can plot with ggplot
...
social <- final_dataset[final_dataset$wave == 4 & final_dataset$country_name == "Estonia", c("iv009_mod", "hhsize", "ch007_km")] ## Create a new social dataset with variables of interest 
...
ggplot() + geom_bar(aes(x = country_wave_dataset$education_band),color="darkblue", fill="lightblue")+
  labs(x ="Education Band", y = "Count") + ggtitle("Education Band Distribution for Estonia") ## Count of values in Education bands
...
labs(x ="Depression Band Index", y = "Count") + ggtitle("Depression Band Distribution for Estonia") ## Count of values in depression bands
...
avg.boot <- averaged.network(boot, threshold = 0.5) ## Create an average DAG
...
```{r}
ci.test("cog_cat", "bmi2", test = "mc-mi", data = data4) ## CI test between cognative category and BMI
``` 
...
bn.bayes = bn.fit(dag7.iamb,data=data5, method = "mle", iss = 10) ## Generate marginal distributions
...
bn.fit.barchart(bn.bayes$cog_cat) ## Generate full conditional probability bar chart
...
```{r}
bn.fit.barchart(bn.bayes$cog_cat, main = "Conditional Probabilities for Risk of Dementia - Estonia", ylab = "Dementia")
...
p1 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
                     y=matrix(t(bn.bayes$cog_cat$prob[,,2,1]), ncol =1), color = rep(levels(country_wave_dataset$cog_cat), each = length(levels(country_wave_dataset$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Estonia\nSecondary Education - Depression Band 1")
p2 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
                     y=matrix(t(bn.bayes$cog_cat$prob[,,2,2]), ncol =1), color = rep(levels(country_wave_dataset$cog_cat), each = length(levels(country_wave_dataset$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Estonia\nSecondary Education - Depression Band 2")
p3 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
  ...
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Estonia\nSecondary Education - Depression Band 3")
p4 = ggplot(mapping = aes(x = rep(seq(1:length(levels(country_wave_dataset$age))),3),
                     y=matrix(t(bn.bayes$cog_cat$prob[,,2,4]), ncol =1), color = rep(levels(country_wave_dataset$cog_cat), each = length(levels(country_wave_dataset$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(country_wave_dataset$age))),
        labels=levels(country_wave_dataset$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Marginal Probability Distribution - Estonia\nSecondary Education - Depression Band 4")
